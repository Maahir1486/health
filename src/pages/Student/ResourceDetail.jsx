...
import { ArrowLeft, Calendar, ExternalLink, PlayCircle, FileText, BookOpen } from 'lucide-react';
...
const ResourceDetail = () => {
    ...
    useEffect(() => {
        // Collect all possible resource sources
        const adminSaved = localStorage.getItem('wellnest_resources');
        const adminResources = adminSaved ? JSON.parse(adminSaved) : initialResources;
        
        const studentSaved = localStorage.getItem(`wellnest_student_resources_${user?.id}`);
        const studentResources = studentSaved ? JSON.parse(studentSaved) : [];
        
        const allResources = [...adminResources, ...studentResources];
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white rounded-3xl p-8 md:p-12 shadow-xl shadow-slate-200/50 border border-slate-100">
                <div className="flex flex-col md:flex-row gap-8 items-start">
                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex-shrink-0 animate-pulse-slow">
