...
import { Sparkles, Activity, TrendingUp, ArrowRight, Droplets, Plus, PlayCircle, FileText, BookOpen } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const [selectedMood, setSelectedMood] = useState(() => {
        return localStorage.getItem(`wellnest_mood_${user.id}`) || '3'; // Default to emoji index 3
    });
    const moods = [
    ...
    const [waterIntake, setWaterIntake] = useState(() => {
        return Number(localStorage.getItem(`wellnest_water_${user.id}`)) || 0;
    });
    const waterGoal = 8; // 8 glasses

    // Resources State (Pick 2 random recommendations)
    ...
    useEffect(() => {
        ...
        const resources = saved ? JSON.parse(saved) : initialResources;
        // Just take the first 2 or shuffle
        setRecommendedResources(resources.slice(0, 2));
    }, []);

    useEffect(() => {
        localStorage.setItem(`wellnest_mood_${user.id}`, selectedMood);
    }, [selectedMood, user.id]);

    useEffect(() => {
        localStorage.setItem(`wellnest_water_${user.id}`, waterIntake);
    }, [waterIntake, user.id]);
        ...
        <div className="space-y-6 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <p className="text-3xl font-bold font-heading text-slate-900">
                        {moods[Number(selectedMood)].label}
                    </p>
                    ...
                    <div className="mt-4 flex gap-2">
                        {moods.map((m, i) => (
                            <button 
                                key={i} 
                                onClick={() => setSelectedMood(String(i))}
                                className={`flex-1 py-1 rounded-lg text-xl transition-all transform hover:scale-110 active:scale-95 ${Number(selectedMood) === i 
